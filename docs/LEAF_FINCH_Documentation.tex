...
\vfill
\noindent\textbf{Associated manuscript (provisionally accepted):}\\
Vijayakumar Anand, Rafał Stojek, and Rafał Kotyński, ``Learned binary amplitude mask designs for Fresnel incoherent correlation holography,'' \emph{Optics Letters} (2026) (submitted; provisionally accepted).
...
\section{Relation to Lee holography and superpixel methods}
In \software{}, the diffraction image is optimized directly at a selected angle and distance from the binary-amplitude DMD. This differs from Lee holography, where an amplitude hologram is encoded from an original complex field multiplied by a linear Lee carrier and subsequently projected onto the binary coding domain. Lee holography normally requires Fourier-plane spatial filtering to retain the selected diffraction order. Spatial filtering may also be useful with \software{} to isolate one of several DMD diffraction orders, but it is not required by the direct optimization method.

The deterministic FZP target provides a Lee-encoded reference implementation related to V. Anand and R. Kotyński, ``Experimental full-field Fresnel incoherent correlation holography using a digital micromirror device,'' \emph{Applied Optics} \textbf{65}, 5479--5490 (2026). The cosine target is the mode used in the accompanying \emph{Optics Letters} manuscript.

In superpixel methods, an amplitude mask is likewise derived from a desired complex field. By contrast, \software{} optimizes the binary mask directly through the propagation model. Direct optimization is substantially more computationally demanding, but the nonparaxial Rayleigh--Sommerfeld model avoids the angular aberrations associated with a simple Fresnel design at larger observation angles and permits high-angular-resolution optimization near the DMD specular-reflection direction, where diffraction efficiency is highest. A representative superpixel reference is \textit{ S. A. Goorden, J. Bertolotti, and A. P. Mosk, “Superpixel-based spatial amplitude and phase modulation using a digital micromirror device,” Opt. Express 22, 17999–18009 (2014).}

The configuration parameters $L$, $2z_r$, and $R_{\mathrm{disk}}$ refer to a lens-free geometry without magnification: $L$ is the DMD-center-to-target-center distance, $2z_r$ characterizes the curvature separation of the interfering waves at the target, and $R_{\mathrm{disk}}$ is the target radius. Fourier lenses, spatial filters, and imaging magnification can change the effective interpretation of these parameters and must be included separately when mapping a laboratory system to the numerical model.
...
\end{equation}
Positive and negative distances are exported as forward and backward propagation, respectively. The generated hologram PDF labels the corresponding intensity panels ``Fresnel forward propagation (Intensity)'' and ``Fresnel backward propagation (Intensity)''. Both intensity and phase panels use the local observation-plane coordinates $u$ and $v$ in millimetres.

\section{Graphical user interface}
The GUI is launched with \texttt{leaf-finch-gui} or \texttt{python run\_gui.py}. Its three tabs are summarized in \cref{tab:gui}.
\begin{table}[H]
\centering
\caption{GUI tabs.}
\label{tab:gui}
...
Tab & Main functions \\
\midrule
Parameters & Device and memory policy, DMD, observation geometry, illumination, target, loss weights, training, reconstruction, and output settings; configuration load/save; model load. \\
...
\caption{Generated binary masks encoding three phase-shifted DMD patterns.}
\label{fig:gui-patterns}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{assets/screenshots/results_reconstructed.png}
\caption{Reconstructed intensity and phase in the circular off-axis target area. The phase-shifted target images jointly encode a complex-valued FINCH hologram.}
\label{fig:gui-reconstruction}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{assets/screenshots/finch_psf.png}
\caption{Reconstructed PSF of the complex FINCH hologram obtained by phase-shifted hologram reconstruction followed by Fresnel backward propagation.}
\label{fig:gui-psf}
\end{figure}
...
\end{lstlisting}
The module form \texttt{python -m leaf\_finch} and direct launcher \texttt{python run\_cli.py} are equivalent. The project homepage, source code, issue tracker, and current documentation are available at \url{https://github.com/rkotynski/leaf_finch}.
...
\begin{lstlisting}[language=bash]
git clone https://github.com/rkotynski/leaf_finch.git
cd leaf_finch
python -m venv .venv
source .venv/bin/activate
# Install the appropriate CPU/CUDA/ROCm PyTorch build first.
pip install -e .
python -m pytest
leaf-finch-gui
\end{lstlisting}
The repository includes a GitHub Actions workflow for CPU tests on supported Python versions. The project homepage is \url{https://github.com/rkotynski/leaf_finch}.

The accompanying manuscript has been provisionally accepted for \emph{Optics Letters}. Until final bibliographic data are available, use the provisional citation stored in \texttt{CITATION.cff} and \texttt{CITATION.bib}:
\begin{quote}
Vijayakumar Anand, Rafał Stojek, and Rafał Kotyński, ``Learned binary amplitude mask designs for Fresnel incoherent correlation holography,'' \emph{Optics Letters} (2026) (submitted; provisionally accepted).
\end{quote}
...
\subsection{License and contributions}
\software{} is distributed under the MIT License. The license permits academic, commercial, and private use, modification, redistribution, and sublicensing, provided that the copyright and license notices are retained. The software is provided without warranty. Bug reports and contributions are welcome through the issue tracker and pull requests at \url{https://github.com/rkotynski/leaf_finch}.
