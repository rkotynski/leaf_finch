...
\begin{itemize}
  ...
  \item a circular optimization domain in an arbitrarily directed plane  (RS propagation prevents creating aberrations that would typically ocure for a simple and a lot faster Fresnel propagation);
...
If \software{} contributes to a publication, use the citation stored in \texttt{CITATION.cff} and \texttt{CITATION.bib}:
\begin{quote}
Vijayakumar Anand, Rafał Stojek, and Rafał Kotyński, ``Learned binary amplitude mask designs for Fresnel incoherent correlation holography,'' \emph{Optics Letters} (2026).
\end{quote}
